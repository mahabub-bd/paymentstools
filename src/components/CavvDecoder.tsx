...
const CavvDecoder = ({ className = '' }) => {
  ...
  return (
    <div className={`w-full bg-white dark:bg-black border border-slate-200 dark:border-zinc-800 rounded-lg p-3 ${className}`}>
      {/* Header */}
      <div className="mb-3">
        <h1 className="text-base font-bold text-slate-800 dark:text-white mb-1">
          CAVV Decoder
        </h1>
      ...
      <div className="mb-3">
        ...
        <textarea
          ...
          className="w-full px-3 py-2 border border-slate-300 dark:border-zinc-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-sm bg-white dark:bg-zinc-900 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-zinc-500"
          rows={2}
        />
        {error && (
          <div className="mt-1.5 text-red-600 dark:text-red-400 text-sm flex items-center gap-1.5">
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <button
          ...
          className="px-4 py-2 bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors active:scale-[0.98]"
        >
          Decode CAVV
        </button>
        <button
          onClick={handleExample}
          className="px-4 py-2 bg-white dark:bg-zinc-900 text-slate-700 dark:text-slate-200 border border-slate-300 dark:border-zinc-700 rounded-md hover:bg-slate-50 dark:hover:bg-zinc-800 text-sm transition-colors"
        >
          Load Example
        </button>
        <button
      ...
      {result && (
        <div className="space-y-2">
          {/* Summary Card */}
          <div className="p-2.5 bg-slate-50 dark:bg-zinc-900 rounded-lg border border-slate-200 dark:border-zinc-800">
            <h3 className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Summary</h3>
            <div className="space-y-1.5">
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  <span className="text-slate-500 dark:text-slate-400 text-xs">Algorithm</span>
                  <div className="font-medium text-slate-900 dark:text-slate-100">{result.algorithm}</div>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400 text-xs">Length</span>
          ...
          {/* Raw Value */}
          <div className="p-2.5 bg-slate-50 dark:bg-zinc-900 rounded-lg border border-slate-200 dark:border-zinc-800">
            <h3 className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Raw CAVV Value</h3>
            <pre className="font-mono text-xs text-slate-800 dark:text-slate-200 break-all bg-white dark:bg-zinc-800 p-1.5 rounded border border-slate-200 dark:border-zinc-700">
          ...
          {/* Decoded Fields */}
          <div className="p-2.5 bg-slate-50 dark:bg-zinc-900 rounded-lg border border-slate-200 dark:border-zinc-800">
            <h3 className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Decoded Fields</h3>
            <div className="space-y-1.5">
              {result.fields.map((field, index) => (
                <div
                  key={index}
                  className="p-2 bg-white dark:bg-zinc-800 rounded border border-slate-200 dark:border-zinc-700"
                >
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <h4 className="text-xs font-semibold text-slate-800 dark:text-slate-200">{field.name}</h4>
                    {field.bits && (
                      <span className="text-xs text-slate-500 dark:text-zinc-500">{field.bits} bits</span>
                    )}
                  </div>
                  <div className="font-mono text-xs text-blue-600 dark:text-blue-400 mb-0.5 break-all">
          ...
          <div className="p-2.5 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
            <h3 className="text-sm font-medium text-amber-800 dark:text-amber-200 mb-1.5">CAVV Field Reference</h3>
