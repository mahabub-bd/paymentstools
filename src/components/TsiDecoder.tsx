...
type DecodeResult = {
  value: string;
  bytes: string[];
  bits: Array<TsiBit & { active: boolean }>;
  activeBits: TsiBit[];
};

const TSI_BITS: TsiBit[] = [
  { byte: 1, bit: 8, label: 'Offline data authentication was performed', category: 'Processing Status' },
  { byte: 1, bit: 7, label: 'Issuer authentication was performed', category: 'Processing Status' },
  { byte: 1, bit: 6, label: 'Cardholder verification was performed', category: 'Processing Status' },
  { byte: 1, bit: 5, label: 'Card risk management was performed', category: 'Processing Status' },
  { byte: 1, bit: 4, label: 'Terminal risk management was performed', category: 'Processing Status' },
  { byte: 1, bit: 3, label: 'RFU', category: 'Reserved for Future Use' },
  { byte: 1, bit: 2, label: 'RFU', category: 'Reserved for Future Use' },
  { byte: 1, bit: 1, label: 'RFU', category: 'Reserved for Future Use' },
];

const EXAMPLES = [
  { value: '80', label: 'Offline data auth performed' },
  { value: 'E0', label: 'All processing performed' },
  { value: '00', label: 'No processing performed' },
  { value: '9B 01 E0', label: 'Tag 9B TLV value' },
  { value: '9B 01 80', label: 'Tag 9B with offline auth only' },
];

const cleanTsiInput = (input: string) => {
  const hex = input.toUpperCase().replace(/[^0-9A-F]/g, '');

  if (hex.startsWith('9B01') && hex.length >= 6) {
    return hex.slice(4, 6);
  }

  if (hex.startsWith('9B') && hex.length >= 4) {
...
const decodeTsi = (input: string): DecodeResult | null => {
  const value = cleanTsiInput(input);
  if (value.length !== 2) return null;

  const bytes = value.match(/.{2}/g) || [];
  const bits = TSI_BITS.map(bitDef => {
...
const groupedBits = TSI_BITS.reduce<Record<string, TsiBit[]>>((groups, bit) => {
  if (!groups[bit.category]) groups[bit.category] = [];
  groups[bit.category].push(bit);
  return groups;
}, {});

interface TsiDecoderProps {
  className?: string;
}

const TsiDecoder = ({ className = '' }: TsiDecoderProps) => {
  const [input, setInput] = useState('');
  const [decoded, setDecoded] = useState<DecodeResult | null>(null);
  const [showAllBits, setShowAllBits] = useState(false);
  const cleanedValue = cleanTsiInput(input);
  const isComplete = cleanedValue.length === 2;
    ...
    <div className={`w-full bg-white dark:bg-black border border-slate-200 dark:border-zinc-800 rounded-lg p-4 ${className}`}>
      <div className="mb-4">
        ...
        <p className="text-slate-600 dark:text-slate-400 text-xs">
          Decode EMV Transaction Status Information, a 1-byte bit field from tag 9B
        </p>
      </div>
      ...
      <div className="mb-4">
          ...
          <div className="relative flex-1 min-w-0">
            <input
              ...
              onChange={(e) => {
                ...
                setInput(next);
                setDecoded(decodeTsi(next));
              }}
              placeholder="E0 or 9B 01 E0"
        ...
        <p className="mt-1 text-[11px] text-slate-500 dark:text-zinc-500">Raw TSI is 1 byte / 2 hex characters.</p>
      </div>
      ...
      {input && !isComplete && (
        <div className="mb-4 p-3 rounded-md border border-amber-200 dark:border-amber-900/60 bg-amber-50 dark:bg-amber-950/20 text-amber-800 dark:text-amber-300 text-xs">
          Enter a complete 1-byte TSI value. Raw TSI values are 2 hex characters.
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="p-3 bg-slate-50 dark:bg-zinc-900 rounded-lg border border-slate-200 dark:border-zinc-800">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:text-zinc-500">Decoded TSI</p>
            ...
            {decoded.activeBits.length === 0 && (
              <p className="mt-2 text-xs text-slate-500 dark:text-zinc-500">No TSI flags are set.</p>
            )}
          </div>
          ...
          <div className="border border-slate-200 dark:border-zinc-800 rounded-lg overflow-hidden">
            <div className="px-3 py-2 bg-gradient-to-r from-slate-50 to-slate-100 dark:from-zinc-900 dark:to-zinc-900/50 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">TSI Bit Results</h3>
              <span className="text-xs text-slate-500 dark:text-zinc-500">Tag 9B format: b1</span>
            </div>
